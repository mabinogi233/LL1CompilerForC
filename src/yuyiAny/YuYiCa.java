...
public class YuYiCa {
     ...
     *          n=-1时为执行产生式末尾的语义动作。
     */
    public static void yuyiAny(LTreeNode father, LTreeNode node, CA ca,int n){
        ErrorFlag = false;
        TypeErrorFlag = false;
        //创建全部文法符号
        ...
        if(ca.equals(createCA(guanXiYuJv6,new ArrayList<MyChar>(){{
            ...
            if(n==0){
                if(node.getToken().getId()!=20){
                    ErrorFlag = true;
                }
            }
            if(n==2){
                if(node.getToken().getId()!=21){
                    ErrorFlag = true;
                }
            }
            if(n==-1) {
     ...
     * @param ca
     */
    public static void yuyiAny(LTreeNode node, CA ca){
